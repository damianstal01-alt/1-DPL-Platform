...
::selection{background:#1A1916;color:#fff}
...
.sb-item{
  ...
  border-left:2.5px solid transparent;transition:all .12s;border-radius:0 6px 6px 0;margin:1px 6px 1px 0;
}
...
.sb-action.danger{color:var(--red)}
.sb-action.danger:hover{background:var(--red-bg)}
...
.f input,.f select,.f textarea{
  ...
  font-family:'DM Sans',sans-serif;width:100%;transition:border-color .12s
}
...
.f .hint{font-size:11px;color:var(--text3);margin-top:2px}
...
.btn{
  ...
  color:var(--text);font-family:'DM Sans',sans-serif;transition:all .12s;white-space:nowrap
}
.btn:hover{background:var(--surface2)}
.btn-primary{background:var(--text);color:#fff;border-color:var(--text)}.btn-primary:hover{opacity:.85}
.btn-green{background:var(--green-bg);color:var(--green);border-color:var(--green-border)}.btn-green:hover{background:#ddf0e8}
.btn-red{background:var(--red-bg);color:var(--red);border-color:var(--red-border)}.btn-red:hover{background:#f5dede}
...
.tbl-actions{display:flex;gap:5px;opacity:0;transition:opacity .12s}
table.tbl tr:hover .tbl-actions{opacity:1}
...
.stop-card.kaan-card{background:var(--blue-bg);border-color:var(--blue-border)}
...
.rnode-depot{padding:4px 10px;border-radius:6px;font-size:12px;font-weight:700;background:var(--blue-bg);color:var(--blue)}
.rnode-stop{padding:4px 10px;border-radius:6px;font-size:12px;background:var(--surface);border:1px solid var(--border);color:var(--text)}
...
/* ── DROPZONE ── */
.dropzone{
  background:var(--surface);border:2px dashed var(--border);border-radius:var(--r);
...
.api-bar input:focus{outline:none;border-color:var(--blue)}
...
.scania-title{font-size:13.5px;font-weight:700;color:var(--green);margin-bottom:8px}

/* ── SEARCH ── */
.search-input{padding:8px 12px;border:1px solid var(--border);border-radius:7px;font-size:13.5px;width:100%;background:var(--surface);color:var(--text);font-family:'DM Sans',sans-serif}
...
@media(max-width:720px){
  ...
  .g2,.g3,.g4,.kpi-grid,.be-grid,.veeke-stats{grid-template-columns:1fr}
...
<nav class="sidebar">
  ...
  <div class="sb-item on" onclick="nav('inst')"><span class="ico">⚙️</span>Instellingen</div>
  <div class="sb-item" onclick="nav('kl')"><span class="ico">👥</span>Klanten</div>
  <div class="sb-item" onclick="nav('week')"><span class="ico">🚛</span>Weekinvoer</div>
  <div class="sb-div"></div>
  <div class="sb-section">Financieel</div>
  <div class="sb-item" onclick="nav('dash')"><span class="ico">📊</span>Dashboard</div>
  <div class="sb-item" onclick="nav('fact')"><span class="ico">🧾</span>Facturen</div>
  <div class="sb-div"></div>
  <div class="sb-bottom">
    <div class="sb-action" onclick="exportData()">⬇ Data exporteren</div>
    <div class="sb-action" onclick="document.getElementById('import-file').click()">⬆ Data importeren</div>
...
<div class="g4">
  ...
  <div class="f"><label>Rijdagen/week</label><input type="number" id="s_huur_dgn" placeholder="2" min="1" max="7" oninput="calcVaste()"></div>
  <div></div>
</div></div>
...
<div style="font-size:13px;color:var(--text2)">Vaste lasten/week (excl. transport): <strong id="vaste-w">—</strong></div>
</div>

<div class="card"><div class="card-t">Commissie Kaan</div>
<div style="max-width:220px">
  <div class="f"><label>Per vracht (EUR excl.)</label><input type="number" id="s_comm" placeholder="100"></div>
</div></div>
...
<div id="prows"></div>
<button class="btn btn-sm" onclick="addP()" style="margin-top:8px">+ Pallettype</button>
</div>
...
<div class="hint-box" style="margin-top:8px">Gratis via openrouteservice.org → Dashboard → API key</div>
</div>
...
<div class="page-header">
  ...
  <div class="page-sub">Adresgegevens worden gebruikt voor routeberekening</div>
</div>
...
<table class="tbl"><thead><tr>
  <th style="width:36px"></th><th>Bedrijfsnaam</th><th>Contactpersoon</th>
  <th>Adres</th><th>Std. pallet</th><th>Notities</th><th style="width:110px"></th>
</tr></thead>
...
<div class="page-header">
  ...
  <div class="page-sub">Route: Depot → stops → Depot</div>
</div>
...
<div class="day-block">
  <div class="day-head" onclick="togD(1)">
    <div class="day-head-l">Dag 1 <span class="badge b-blue" id="km1">0 km</span></div>
    <div style="display:flex;align-items:center;gap:8px">
      <button class="btn btn-sm" onclick="event.stopPropagation();doRoute(1)">📍 Route</button>
      <span id="chev1" style="color:var(--text3)">▾</span>
    </div>
  </div>
  <div class="day-body" id="db1">
    <div id="rv1" class="route-vis" style="display:none"></div>
    <div id="st1"></div>
    <div style="margin-top:10px"><button class="btn btn-sm" onclick="addStop(1)">+ Levering toevoegen</button></div>
    <div id="rr1" style="display:none;margin-top:10px"></div>
  </div>
</div>

<div class="day-block">
  <div class="day-head" onclick="togD(2)">
    <div class="day-head-l">Dag 2 <span class="badge b-blue" id="km2">0 km</span></div>
    <div style="display:flex;align-items:center;gap:8px">
      <button class="btn btn-sm" onclick="event.stopPropagation();doRoute(2)">📍 Route</button>
      <span id="chev2" style="color:var(--text3)">▾</span>
    </div>
  </div>
  <div class="day-body" id="db2">
    <div id="rv2" class="route-vis" style="display:none"></div>
    <div id="st2"></div>
    <div style="margin-top:10px"><button class="btn btn-sm" onclick="addStop(2)">+ Levering toevoegen</button></div>
...
<div class="page-header">
  ...
  <div class="page-sub">Winst, kosten en trends in één oogopslag</div>
</div>
...
<div style="font-size:12px;color:var(--text3)" id="be-hint"></div>
</div>
...
<div class="scania-box" id="d-sc" style="display:none">
  <div class="scania-title">🚛 Scania overgangsmodule</div>
...
<div class="page-header">
  ...
  <div class="page-sub">Sleep een PDF — bedrag en vervaldatum worden automatisch uitgelezen</div>
</div>

<div class="kpi-grid">
  <div class="kpi"><div class="kpi-label">Openstaand</div><div class="kpi-value" id="fm1">—</div></div>
  <div class="kpi red"><div class="kpi-label">Vervallen</div><div class="kpi-value" id="fm2" style="color:var(--red)">—</div></div>
  <div class="kpi orange"><div class="kpi-label">Vervalt deze week</div><div class="kpi-value" id="fm3" style="color:var(--orange)">—</div></div>
  <div class="kpi green"><div class="kpi-label">Betaald (30d)</div><div class="kpi-value" id="fm4" style="color:var(--green)">—</div></div>
</div>
...
<div class="dropzone" id="fdz">
  ...
  <div class="dz-sub">of klik om te bladeren — bedrag en vervaldatum worden automatisch uitgelezen via AI</div>
</div>
...
<div class="fact-card">
<table class="tbl"><thead><tr>
  <th>Leverancier</th><th>Factuurnr.</th><th>Bedrag excl.</th><th>Vervaldatum</th><th>Status</th><th></th>
</tr></thead>
...
<div class="modal">
  ...
  <div class="f mb"><label>Leverancier</label><input class="f" id="fflev" placeholder="bv. Veeke Palletreparatie" style="width:100%;padding:8px 11px;border:1px solid var(--border);border-radius:7px;font-size:13.5px;font-family:'DM Sans',sans-serif"></div>
  <div class="g2 mb">
    <div class="f"><label>Factuurnummer</label><input type="text" id="ffnr" placeholder="F01099" style="padding:8px 11px;border:1px solid var(--border);border-radius:7px;font-size:13.5px;font-family:'DM Sans',sans-serif;width:100%"></div>
    <div class="f"><label>Bedrag excl. btw (€)</label><input type="number" id="ffbed" placeholder="0.00" step="0.01" min="0" style="padding:8px 11px;border:1px solid var(--border);border-radius:7px;font-size:13.5px;font-family:'DM Sans',sans-serif;width:100%"></div>
  </div>
  <div class="g2 mb">
    <div class="f"><label>Factuurdatum</label><input type="date" id="ffdat" style="padding:8px 11px;border:1px solid var(--border);border-radius:7px;font-size:13.5px;font-family:'DM Sans',sans-serif;width:100%"></div>
    <div class="f"><label>Vervaldatum</label><input type="date" id="ffver" style="padding:8px 11px;border:1px solid var(--border);border-radius:7px;font-size:13.5px;font-family:'DM Sans',sans-serif;width:100%"></div>
  </div>
  <div class="f mb"><label>Notitie</label><input type="text" id="ffnot" placeholder="optioneel" style="padding:8px 11px;border:1px solid var(--border);border-radius:7px;font-size:13.5px;font-family:'DM Sans',sans-serif;width:100%"></div>
...
// NAVIGATION
// ══════════════════════════════════════════════
var PAGES=['inst','kl','week','dash','fact'];
var PAGE_IDS={'inst':'pg-inst','kl':'pg-kl','week':'pg-week','dash':'pg-dash','fact':'pg-fact'};
var NAV_SEL={'inst':0,'kl':1,'week':2,'dash':3,'fact':4};

function nav(id){
  PAGES.forEach(function(p){
    document.getElementById(PAGE_IDS[p]).classList.remove('on');
  });
  document.getElementById(PAGE_IDS[id]).classList.add('on');
  document.querySelectorAll('.sb-item').forEach(function(el,i){el.classList.remove('on')});
  document.querySelectorAll('.sb-item')[NAV_SEL[id]].classList.add('on');
...
var fRows=[], fFilter='open', fBusy=false, fEditId=null;

var DEF={
  startloc:'Havenkant 2, 4781AA Moerdijk',
  winstdoel:4000, diesel:1.75, wegbel:0.20,
  huur_dag:230, huur_vbr:28, huur_dgn:2,
  sc_lease:1850, sc_ondh:200, sc_vbr:28,
  trailer:500, verz:500, admin:600, overig:0, comm:100,
  pallets:[
    {naam:'EPAL Europallet',inkoop:2.50,rep:3.25,verkoop:7.50},
    {naam:'Blokpallet',inkoop:0.60,rep:0,verkoop:3.75},
    {naam:'80/120',inkoop:0.60,rep:0,verkoop:3.25},
    {naam:'CP3',inkoop:8.00,rep:0,verkoop:10.50},
    {naam:'102/120',inkoop:5.00,rep:0,verkoop:7.50}
  ]
};
...
function fe(v){return'€\u202f'+Math.abs(v).toLocaleString('nl-NL',{minimumFractionDigits:2,maximumFractionDigits:2})}
...
function wkNum(d){var j=new Date(Date.UTC(d.getFullYear(),d.getMonth(),d.getDate()));j.setUTCDate(j.getUTCDate()+4-(j.getUTCDay()||7));return Math.ceil(((j-new Date(Date.UTC(j.getUTCFullYear(),0,1)))/86400000+1)/7)}
...
function loadS(){
  var s=GS();
  sv('s_startloc',s.startloc||'');sv('s_winstdoel',s.winstdoel||'');sv('s_diesel',s.diesel||'');
  sv('s_wegbel',s.wegbel||'');sv('s_huur_dag',s.huur_dag||'');sv('s_huur_vbr',s.huur_vbr||'');
  sv('s_huur_dgn',s.huur_dgn||'');sv('s_sc_datum',s.sc_datum||'');sv('s_sc_lease',s.sc_lease||'');
  sv('s_sc_ondh',s.sc_ondh||'');sv('s_sc_vbr',s.sc_vbr||'');sv('s_trailer',s.trailer||'');
  sv('s_verz',s.verz||'');sv('s_admin',s.admin||'');sv('s_overig',s.overig||'');sv('s_comm',s.comm||'');
...
function saveS(){
  var s={
    ...
    overig:parseFloat(gv('s_overig'))||0,comm:parseFloat(gv('s_comm'))||100,
    pallets:pallets
  };
  localStorage.setItem('pl_s',JSON.stringify(s));
  var ok=document.getElementById('s-ok');ok.style.display='block';
...
function calcVaste(){
  var s={
    huur_dag:parseFloat(gv('s_huur_dag'))||parseFloat(GS().huur_dag)||230,
    huur_dgn:parseInt(gv('s_huur_dgn'))||parseInt(GS().huur_dgn)||2,
    sc_datum:gv('s_sc_datum')||GS().sc_datum||'',
    sc_lease:parseFloat(gv('s_sc_lease'))||parseFloat(GS().sc_lease)||1850,
    sc_ondh:parseFloat(gv('s_sc_ondh'))||parseFloat(GS().sc_ondh)||200,
    trailer:parseFloat(gv('s_trailer'))||parseFloat(GS().trailer)||0,
    verz:parseFloat(gv('s_verz'))||parseFloat(GS().verz)||0,
    admin:parseFloat(gv('s_admin'))||parseFloat(GS().admin)||0,
    overig:parseFloat(gv('s_overig'))||parseFloat(GS().overig)||0
  };
  var isS=s.sc_datum&&new Date(s.sc_datum)<=new Date();
  var vMnd=s.trailer+s.verz+s.admin+s.overig+(isS?(s.sc_lease+s.sc_ondh):0);
  var vWk=vMnd/4.33;
  var el=document.getElementById('vaste-w');
  if(el)el.textContent='€'+vWk.toFixed(2)+'/week (€'+Math.round(vMnd).toLocaleString('nl-NL')+'/mnd)';
  // Scania preview
  var prev=document.getElementById('sc-prev');
  if(prev&&s.sc_datum){
    var d=new Date(s.sc_datum),nu=new Date();
    if(d>nu){
      var dag=Math.ceil((d-nu)/86400000);
      var hm=(s.huur_dag)*(s.huur_dagn||2)*4.33;
      var sm=s.sc_lease+s.sc_ondh;
      prev.innerHTML='Verwacht over <strong>'+dag+' dagen</strong>. Huurwagen nu: ca. €'+Math.round(s.huur_dag*s.huur_dgn*4.33)+'/mnd → Scania: €'+Math.round(sm)+'/mnd. Besparing: <strong style="color:var(--green)">€'+Math.round(s.huur_dag*s.huur_dgn*4.33-sm)+'/mnd</strong>';
    } else {
      prev.innerHTML='<strong style="color:var(--green)">✓ Scania actief</strong>';
    }
  }
}

function renderP(){
  var c=document.getElementById('prows');if(!c)return;
  c.innerHTML='';
  pallets.forEach(function(p,i){
    ...
    d.innerHTML='<div class="f"><input type="text" value="'+esc(p.naam)+'" placeholder="Naam" oninput="pallets['+i+'].naam=this.value"></div>'
      ...
      +'<div class="f"><input type="number" step="0.01" value="'+(p.rep||'')+'" placeholder="0.00" oninput="pallets['+i+'].rep=parseFloat(this.value)||0"></div>'
      ...
      +'<button class="btn btn-sm btn-red" onclick="pallets.splice('+i+',1);renderP()" title="Verwijder" style="padding:6px 8px">✕</button>';
    c.appendChild(d);
  });
}
function addP(){pallets.push({naam:'',inkoop:0,rep:0,verkoop:0});renderP()}
function kp(naam){var p=getPals().find(function(x){return x.naam===naam});return p?p.inkoop:0}

// ══════════════════════════════════════════════
// KLANTEN
// ══════════════════════════════════════════════
function renderK(){
  var cls=GC(),q=(gv('kl-s')||'').toLowerCase(),list=q?cls.filter(function(c){return(c.bedrijf+c.contactpersoon+c.plaats).toLowerCase().includes(q)}):cls;
  ...
  if(tb)tb.innerHTML=list.map(function(c,i){
    var ini=(c.bedrijf||'?')[0].toUpperCase();
    var col=cols[i%cols.length];
    return'<tr><td><div style="width:32px;height:32px;border-radius:50%;background:'+col+';display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;color:var(--text2)">'+ini+'</div></td>'
      +'<td><strong>'+esc(c.bedrijf)+'</strong></td>'
      +'<td>'+(c.contactpersoon?esc(c.contactpersoon):'<span style="color:var(--text3)">—</span>')+'</td>'
      +'<td style="font-size:12px">'+(c.straat?esc(c.straat+', '+c.plaats):'<span style="color:var(--text3)">—</span>')+'</td>'
      +'<td>'+(c.pallet?'<span class="badge b-blue">'+esc(c.pallet)+'</span>':'<span style="color:var(--text3)">—</span>')+'</td>'
      +'<td style="font-size:12px;color:var(--text3)">'+(c.notities?esc(c.notities.slice(0,40)):'')+'</td>'
      +'<td><div class="tbl-actions"><button class="btn btn-xs" onclick="editK(\''+c.id+'\')">Bewerk</button><button class="btn btn-xs btn-red" onclick="delK(\''+c.id+'\')">✕</button></div></td>'
      +'</tr>';
  }).join('');
}
function openKM(id){
  ...
  ['kf_bedr','kf_cont','kf_tel','kf_email','kf_str','kf_pl','kf_pc','kf_not'].forEach(function(fid){sv(fid,c?(c[{kf_bedr:'bedrijf',kf_cont:'contactpersoon',kf_tel:'telefoon',kf_email:'email',kf_str:'straat',kf_pl:'plaats',kf_pc:'postcode',kf_not:'notities'}[fid]]||''):'')});
  sv('kf_land',c?c.land||'NL':'NL');
  sv('kf_ant',c?c.aantal||'':'');
  var ps=document.getElementById('kf_pal');
  ps.innerHTML='<option value="">— geen voorkeur —</option>';
...
function editK(id){openKM(id)}
function delK(id){if(!confirm('Klant verwijderen?'))return;var cls=GC().filter(function(c){return c.id!==id});SC(cls);renderK()}
function saveK(){
  ...
  var obj={id:editKId||(Date.now().toString()),bedrijf:bedr,contactpersoon:gv('kf_cont'),telefoon:gv('kf_tel'),email:gv('kf_email'),straat:str,plaats:pl,postcode:gv('kf_pc'),land:gv('kf_land'),pallet:gv('kf_pal'),aantal:parseInt(gv('kf_ant'))||0,notities:gv('kf_not')};
  var cls=GC();
...
function updV(){
  ...
  if(lg)lg.textContent=hui>=0?'Eind stand: '+hui+' pallets bij Veeke':'⚠ Negatieve stand — controleer invoer';
}
function togD(d){
  var b=document.getElementById('db'+d),ch=document.getElementById('chev'+d);
  dOpen[d]=!dOpen[d];
  b.style.display=dOpen[d]?'block':'none';
...
function updRV(dag){
  ...
  rv.innerHTML='<span class="rnode-depot">'+esc(depot)+'</span>';
  stops.forEach(function(s){rv.innerHTML+='<span class="rarr">→</span><span class="rnode-stop '+(s.coords?'rnode-ok':'')+'" title="'+esc(s.adres)+'">'+esc(s.adres.split(',')[0])+'</span>'});
  rv.innerHTML+='<span class="rarr">→</span><span class="rnode-depot">'+esc(depot)+'</span>';
}
function renderSt(dag){
  ...
  days[dag].forEach(function(stop,i){
    var kkp=stop.pallet?(function(){var p=getPals().find(function(x){return x.naam===stop.pallet});return p?p.inkoop:0})():0;
    ...
    div.className='stop-card'+(stop.kaan?' kaan-card':'');
    div.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">'
      ...
      +'<div class="f"><label>Inkoop p/st (EUR)</label><input type="number" step="0.01" min="0" value="'+(stop.inkoop||'')+'" placeholder="0.00" oninput="days['+dag+']['+i+'].inkoop=parseFloat(this.value)||0;herb()">'+(kkp>0?'<span class="f hint">kostprijs: €'+kkp.toFixed(2)+'</span>':'')+'</div>'
      +'<div class="f"><label>Verkoop p/st (EUR)</label><input type="number" step="0.01" min="0" value="'+(stop.verkoop||'')+'" placeholder="0.00" oninput="days['+dag+']['+i+'].verkoop=parseFloat(this.value)||0;herb()">'+(m!==null?'<span style="font-size:11px;padding:2px 7px;border-radius:4px;display:inline-block;margin-top:2px;'+ms+'">'+m+'% marge</span>':'')+'</div>'
      +'</div>'
      ...
      +'<input type="checkbox" '+(stop.kaan?'checked':'')+' onchange="days['+dag+']['+i+'].kaan=this.checked;herb();renderSt('+dag+')" style="width:15px;height:15px"> Geregeld door Kaan</label>'
...
function addStop(dag){days[dag].push({type:'lev',cId:'',pallet:'',aantal:0,inkoop:0,verkoop:0,kaan:false,adres:'',coords:null});renderSt(dag)}
...
function herb(){
  ...
  var dK=totKm*(vbr/100)*diesel,wK=totKm*(parseFloat(s.wegbel)||0.20),hK=isS?0:(parseFloat(s.huur_dag)||230)*(parseInt(s.huur_dgn)||2);
  var tK=dK+wK+hK;
  ...
  var vWk=vMnd/4.33;
  var omz=0,ink=0,com=0;
  [1,2].forEach(function(dag){days[dag].forEach(function(st){omz+=(st.aantal||0)*(st.verkoop||0);ink+=(st.aantal||0)*(st.inkoop||0);if(st.kaan)com+=parseFloat(s.comm)||100})});
  var bruto=omz-ink,totK=tK+vWk+com,netto=bruto-totK,doelW=(parseFloat(s.winstdoel)||4000)/4.33;
  ...
  var win=document.getElementById('w-win');
  win.style.color=netto>=doelW?'var(--green)':netto>0?'var(--orange)':'var(--red)';
  ...
  var rows=[['Omzet','€'+omz.toFixed(2)],['Inkoopkosten','− €'+ink.toFixed(2)],['Brutowinst','€'+bruto.toFixed(2),'','fw'],
    [isS?'Scania lease/week':'Huurwagen','− €'+hK.toFixed(2)],['Diesel ('+totKm+' km)','− €'+dK.toFixed(2)],
    ['Wegenbelasting','− €'+wK.toFixed(2)],['Vaste lasten (weekdeel)','− €'+vWk.toFixed(2)],
    ['Commissie Kaan','− €'+com.toFixed(2)],['Nettowinst','€'+netto.toFixed(2),'','fw']];
...
function slaOp(){
  ...
  localStorage.setItem(key,JSON.stringify({week:wk,jaar:jr,days:days,kmDay:kmD,diesel:gv('w_diesel'),veeke:{begin:parseInt(gv('v_begin'))||0,bracht:parseInt(gv('v_bracht_in'))||0,opgh:parseInt(gv('v_opgh_in'))||0},ts:new Date().toISOString()}));
...
function resetWeek(){
  ...
  var idx=JSON.parse(localStorage.getItem('pl_wi')||'[]');
...
function calcW(w){
  ...
  var isS=s.sc_datum&&new Date(s.sc_datum)<=new Date(w.jaar,0,1+(w.week-1)*7);
  var vbr=parseFloat(isS?s.sc_vbr:s.huur_vbr)||28;
  var dK=totKm*(vbr/100)*diesel,wK=totKm*(parseFloat(s.wegbel)||0.20),hK=isS?0:(parseFloat(s.huur_dag)||230)*(parseInt(s.huur_dgn)||2);
  var vMnd=(parseFloat(s.trailer)||0)+(parseFloat(s.verz)||0)+(parseFloat(s.admin)||0)+(parseFloat(s.overig)||0)+(isS?(parseFloat(s.sc_lease)||0)+(parseFloat(s.sc_ondh)||0):0);
  var vWk=vMnd/4.33;
  var omz=0,ink=0,com=0,nP=0;
  [1,2].forEach(function(dag){(dd[dag]||[]).forEach(function(st){omz+=(st.aantal||0)*(st.verkoop||0);ink+=(st.aantal||0)*(st.inkoop||0);nP+=(st.aantal||0);if(st.kaan)com+=parseFloat(s.comm)||100})});
  var bruto=omz-ink,totK=dK+wK+hK+vWk+com,netto=bruto-totK;
  return{lbl:'W'+w.week+"'"+String(w.jaar).slice(2),week:parseInt(w.week),jaar:parseInt(w.jaar),sk:parseInt(w.jaar)*100+parseInt(w.week),omz:omz,ink:ink,bruto:bruto,tK:dK+wK+hK,vWk:vWk,com:com,totK:totK,netto:netto,marge:omz>0?netto/omz*100:0,totKm:totKm,nP:nP,dK:dK,wK:wK,hK:hK};
}
...
function setFil(f,el){dFil=f;document.querySelectorAll('.fb').forEach(function(b){if(b.closest('#pg-dash'))b.classList.remove('on')});el.classList.add('on');renderDash()}
...
function renderDash(){
  ...
  var domz=document.getElementById('d-omz');if(domz){domz.textContent=fe(tO);domz.parentElement.className='kpi'}
  var dwin=document.getElementById('d-win');if(dwin){dwin.textContent=fe(tW);dwin.parentElement.className='kpi'+(tW>=0?' green':' red')}
  var dmarg=document.getElementById('d-marg');if(dmarg)dmarg.textContent=gM.toFixed(1)+'%';
  var dkm=document.getElementById('d-km');if(dkm)dkm.textContent=tK.toLocaleString('nl-NL')+' km';
  var dos=document.getElementById('d-omz-s');if(dos)dos.textContent='gem. '+fe(tO/weken.length)+'/week';
  var dws=document.getElementById('d-win-s');if(dws)dws.textContent='gem. '+fe(tW/weken.length)+'/week';
  var dms=document.getElementById('d-marg-s');if(dms)dms.textContent='netto over omzet';
  var dks=document.getElementById('d-km-s');if(dks)dks.textContent='gem. '+Math.round(tK/weken.length)+' km/week';
...
function drawTblDash(weken,doelW){
  ...
  el.innerHTML='<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Week</th><th>Omzet</th><th>Inkoop</th><th>Kosten</th><th>Nettowinst</th><th>Marge</th><th>Km</th><th>Status</th></tr></thead><tbody>'
    +weken.slice().reverse().map(function(w){
      var col=w.netto>=doelW?'var(--green)':w.netto>0?'var(--orange)':'var(--red)';
      var dot=w.netto>=doelW?'dot-g':w.netto>0?'dot-o':'dot-r';
      return'<tr><td><strong>'+w.lbl+'</strong></td><td>'+fe(w.omz)+'</td><td>'+fe(w.ink)+'</td><td>'+fe(w.totK)+'</td>'
        +'<td style="font-weight:700;color:'+col+'">'+fe(w.netto)+'</td>'
        +'<td>'+w.marge.toFixed(1)+'%</td><td>'+w.totKm+' km</td>'
        +'<td><span class="dot '+dot+'"></span>'+(w.netto>=doelW?'Op koers':w.netto>0?'Onder doel':'Verlies')+'</td></tr>'
...
function svgBar(weken){
  ...
  [0,.25,.5,.75,1].forEach(function(t){var y=pt+ch*(1-t),v=rMin+range*t;svg+='<line x1="'+pl+'" y1="'+y+'" x2="'+(W-pr)+'" y2="'+y+'" stroke="#E8E8E0" stroke-width="1"/><text x="'+(pl-5)+'" y="'+(y+4)+'" text-anchor="end" font-size="10" fill="#A0A090" font-family="DM Mono,monospace">€'+Math.round(v/1000)+'k</text>'});
  svg+='<line x1="'+pl+'" y1="'+zy+'" x2="'+(W-pr)+'" y2="'+zy+'" stroke="#CCCCB8" stroke-width="1.5"/>';
  weken.forEach(function(w,i){
    ...
    [[w.omz,'#378ADD'],[w.totK,'#F09595'],[w.netto,'#1D9E75']].forEach(function(p,j){
      ...
      if(bh<1)bh=1;svg+='<rect x="'+bx+'" y="'+by+'" width="'+(gbw-2)+'" height="'+bh+'" fill="'+col+'" rx="2" opacity=".9"/>';
    });
    svg+='<text x="'+(x+bw/2-8)+'" y="'+(H-4)+'" text-anchor="middle" font-size="10" fill="#A0A090" font-family="DM Mono,monospace">'+w.lbl+'</text>';
  });
  return svg+'</svg>';
}
...
function svgMarg(weken){
  ...
  [0,.5,1].forEach(function(t){var y=pt+ch*(1-t),v=rMin+range*t;svg+='<line x1="'+pl+'" y1="'+y+'" x2="'+(W-pr)+'" y2="'+y+'" stroke="#E8E8E0" stroke-width="1"/><text x="'+(pl-4)+'" y="'+(y+4)+'" text-anchor="end" font-size="10" fill="#A0A090" font-family="DM Mono,monospace">'+Math.round(v)+'%</text>'});
  if(n>1){
    var pts=weken.map(function(w,i){var x=pl+(i/(n-1))*cw,y=pt+ch*(1-(w.marge-rMin)/range);return x+','+y});
    svg+='<polyline points="'+pts.join(' ')+'" fill="none" stroke="#1D9E75" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>';
    weken.forEach(function(w,i){var x=pl+(i/(n-1))*cw,y=pt+ch*(1-(w.marge-rMin)/range);svg+='<circle cx="'+x+'" cy="'+y+'" r="4" fill="#1D9E75"/>'});
  }
  weken.forEach(function(w,i){var x=pl+(n>1?(i/(n-1))*cw:cw/2);svg+='<text x="'+x+'" y="'+(H-3)+'" text-anchor="middle" font-size="10" fill="#A0A090" font-family="DM Mono,monospace">'+w.lbl+'</text>'});
  return svg+'</svg>';
}
...
function svgDon(weken){
  ...
  var W=300,H=260,cx=130,cy=110,r=85,ri=50,ang=-Math.PI/2;
  var svg='<svg viewBox="0 0 '+W+' '+H+'" xmlns="http://www.w3.org/2000/svg" style="width:100%">';
  ...
  tots.forEach(function(v,i){var lx=10,ly=H-60+i*18;svg+='<rect x="'+lx+'" y="'+(ly-8)+'" width="10" height="10" fill="'+cols[i]+'" rx="2"/><text x="'+(lx+14)+'" y="'+ly+'" font-size="11" fill="#888" font-family="DM Sans,sans-serif">'+lbls[i]+' (€'+Math.round(v).toLocaleString('nl-NL')+')</text>'});
  return svg+'</svg>';
}
...
function svgMnd(all,doelMnd){
  ...
  var svg='<svg viewBox="0 0 '+W+' '+H+'" xmlns="http://www.w3.org/2000/svg" style="width:100%">';
  [0,.5,1].forEach(function(t){var y=pt+ch*(1-t),v=rMin+range*t;svg+='<line x1="'+pl+'" y1="'+y+'" x2="'+(W-pr)+'" y2="'+y+'" stroke="#E8E8E0" stroke-width="1"/><text x="'+(pl-4)+'" y="'+(y+4)+'" text-anchor="end" font-size="10" fill="#A0A090" font-family="DM Mono,monospace">€'+Math.round(v/1000)+'k</text>'});
  var dy=pt+ch*(1-(doelMnd-rMin)/range);if(dy>=pt&&dy<=pt+ch)svg+='<line x1="'+pl+'" y1="'+dy+'" x2="'+(W-pr)+'" y2="'+dy+'" stroke="#2D7A4F" stroke-width="1" stroke-dasharray="4,3"/><text x="'+(W-pr-2)+'" y="'+(dy-3)+'" text-anchor="end" font-size="9" fill="#2D7A4F" font-family="DM Sans,sans-serif">doel</text>';
  svg+='<line x1="'+pl+'" y1="'+zy+'" x2="'+(W-pr)+'" y2="'+zy+'" stroke="#CCCCB8" stroke-width="1.5"/>';
  data.forEach(function(v,i){
    var x=pl+i*bw+bp,bx=Math.max(bw-bp*2,2),col=v>=doelMnd?'#1D9E75':v>0?'#EF9F27':'#E24B4A',by,bh;
    if(v>=0){by=pt+ch*(1-(v-rMin)/range);bh=zy-by}else{by=zy;bh=Math.abs(pt+ch*(1-(v-rMin)/range)-zy)}
    if(bh<1)bh=1;svg+='<rect x="'+x+'" y="'+by+'" width="'+bx+'" height="'+bh+'" fill="'+col+'" rx="2" opacity=".9"/>';
    svg+='<text x="'+(x+bx/2)+'" y="'+(H-4)+'" text-anchor="middle" font-size="9" fill="#A0A090" font-family="DM Mono,monospace">'+lbls[i]+'</text>';
  });
  return svg+'</svg>';
}
...
function loadFact(){
  try{var r=localStorage.getItem('dpl_fact_v1');fRows=r?JSON.parse(r):FACT_INIT.map(function(x){return Object.assign({},x)})}
  catch(e){fRows=FACT_INIT.map(function(x){return Object.assign({},x)})}
}
...
function getFactStatus(f){
  ...
  var v=new Date(f.ver+'T00:00:00');
  if(v<factToday) return 'vervallen';
  if(v<=factWeek7) return 'week';
  return 'open';
}
function factBadge(f){
  var s=getFactStatus(f);
  var m={betaald:['b-green','✓ Betaald'],incasso:['b-blue','Auto-incasso'],vervallen:['b-red','Vervallen'],week:['b-orange','Vervalt deze week'],open:f.ver?['b-blue','Openstaand']:['b-gray','Verval onbekend']};
  var pair=m[s]||['b-gray',s];
  return'<span class="badge '+pair[0]+'">'+pair[1]+'</span>';
}
function factFilterOk(f){
  var s=getFactStatus(f);
  if(fFilter==='all') return true;
  if(fFilter==='betaald') return s==='betaald'||s==='incasso';
  if(fFilter==='vervallen') return s==='vervallen';
  if(fFilter==='week') return s==='week';
  if(fFilter==='open') return s!=='betaald';
  return true;
}
...
function setFFilter(f,btn){
  fFilter=f;
  document.querySelectorAll('#pg-fact .fb').forEach(function(b){b.classList.remove('on')});
  btn.classList.add('on');
  renderFact();
}

function renderFact(){
  var list=fRows.filter(factFilterOk);
  var tot=0,ov=0,wk=0,paid=0;
  fRows.forEach(function(f){
    var s=getFactStatus(f),b=f.bed||0;
  ...
  document.getElementById('fm1').textContent=mfmt(tot);
  document.getElementById('fm2').textContent=mfmt(ov);
  document.getElementById('fm3').textContent=mfmt(wk);
  document.getElementById('fm4').textContent=mfmt(paid);
  var tb=document.getElementById('ftbody'),em=document.getElementById('fempty');
  if(!list.length){tb.innerHTML='';em.style.display='';return}
  em.style.display='none';
  tb.innerHTML=list.map(function(f){
    var s=getFactStatus(f),isPaid=s==='betaald'||s==='incasso';
    return'<tr><td><strong>'+esc(f.lev)+'</strong>'+(f.not?'<div class="note-text">'+esc(f.not)+'</div>':'')+'</td>'
      +'<td><span class="mono">'+esc(f.nr)+'</span></td>'
      +'<td>'+factEuro(f.bed)+'</td>'
      +'<td>'+factDat(f.ver)+'</td>'
      +'<td>'+factBadge(f)+'</td>'
      +'<td style="text-align:right;white-space:nowrap"><div class="tbl-actions" style="opacity:1">'
        ...
        +'<button class="btn btn-xs btn-red" onclick="delFact('+f.id+')">✕</button>'
      +'</div></td></tr>';
  }).join('');
}

function markFactPaid(id){
  var f=fRows.find(function(x){return x.id===id});
  if(f){f.st='betaald';saveFact();renderFact();showToast('Factuur gemarkeerd als betaald')}
}
function delFact(id){
  if(!confirm('Factuur verwijderen?'))return;
  fRows=fRows.filter(function(x){return x.id!==id});
  saveFact();renderFact();
}
function openFactModal(f){
  ...
  document.getElementById('fflev').value=f?f.lev:'';
  document.getElementById('ffnr').value=f?f.nr:'';
  ...
  document.getElementById('ffver').value=f?(f.ver||''):'';
  document.getElementById('ffnot').value=f?(f.not||''):'';
...
function saveFactuur(){
  ...
  var obj={
    lev:document.getElementById('fflev').value.trim(),
    nr:document.getElementById('ffnr').value.trim(),
    bed:parseFloat(document.getElementById('ffbed').value)||null,
    dat:document.getElementById('ffdat').value||null,
    ver:document.getElementById('ffver').value||null,
    not:document.getElementById('ffnot').value.trim(),
    st:'open'
  };
...
// Anthropic API key for facturen
function onFKey(v){
  try{localStorage.setItem('dpl_apikey',v)}catch(e){}
  document.getElementById('f_keystatus').textContent=v.startsWith('sk-ant-')?'✓ Opgeslagen':'';
}
...
async function processPdf(file){
  if(fBusy){showToast('Even wachten, vorige PDF wordt nog verwerkt...');return}
  var key=getFKey();
  if(!key||!key.startsWith('sk-ant-')){showToast('Vul eerst een geldige Anthropic API key in bij Facturen');return}
  fBusy=true;
  fdz.classList.add('busy');
  document.getElementById('fdztitle').textContent=file.name+' verwerken...';
  try{
    var txt=await extractPdfText(file);
    var prompt='Je bent een factuur-parser. Extraheer uit deze factuur:\n- leverancier: naam afzender\n- factuurnummer\n- bedrag: totaal EXCLUSIEF btw als getal\n- factuurdatum: YYYY-MM-DD\n- vervaldatum: YYYY-MM-DD\n\nAntwoord UITSLUITEND met JSON:\n{"leverancier":"...","factuurnummer":"...","bedrag":0.00,"factuurdatum":"YYYY-MM-DD","vervaldatum":"YYYY-MM-DD"}\n\nGebruik null als een veld ontbreekt.\n\nFactuur:\n'+txt.slice(0,3000);
    ...
    var p=JSON.parse((json.content[0]?.text||'{}').replace(/```json|```/g,'').trim());
    ...
    if(existing){
      ...
      saveFact();renderFact();showToast(existing.nr+' bijgewerkt vanuit PDF');
    } else {
      var nw={id:Date.now(),lev:p.leverancier||file.name.replace('.pdf',''),nr:p.factuurnummer||file.name.replace('.pdf',''),bed:p.bedrag||null,dat:p.factuurdatum||null,ver:p.vervaldatum||null,st:'open',not:''};
      fRows.push(nw);saveFact();renderFact();showToast(nw.nr+' toegevoegd — '+nw.lev);
    }
  }catch(e){showToast('Fout: '+e.message);console.error(e)}
  finally{
    fBusy=false;fdz.classList.remove('busy');
    document.getElementById('fdztitle').textContent="Sleep factuur-PDF's hierheen";
    document.getElementById('ffileinput').value='';
  }
}
...
function exportData(){
  ...
  var idx=JSON.parse(localStorage.getItem('pl_wi')||'[]');
  idx.forEach(function(k){data.weeks[k]=localStorage.getItem(k)});
  var blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
  var url=URL.createObjectURL(blob);
  var a=document.createElement('a');a.href=url;
  a.download='palletleverancier-'+new Date().toISOString().slice(0,10)+'.json';
...
function importData(input){
  ...
  reader.onload=function(e){
    try{
      ...
      if(data.settings)localStorage.setItem('pl_s',data.settings);
      if(data.clients)localStorage.setItem('pl_c',data.clients);
      if(data.veeke)localStorage.setItem('pl_veeke',data.veeke);
      if(data.weekIndex)localStorage.setItem('pl_wi',data.weekIndex);
      if(data.facturen)localStorage.setItem('dpl_fact_v1',data.facturen);
      if(data.weeks)Object.keys(data.weeks).forEach(function(k){localStorage.setItem(k,data.weeks[k])});
      alert('✓ Data geïmporteerd. Pagina wordt herladen.');
      location.reload();
...
renderK();
var now=new Date();
sv('w_wk',wkNum(now));sv('w_jr',now.getFullYear());
addStop(1);addStop(2);updV();
</script>
</body>
</html>
